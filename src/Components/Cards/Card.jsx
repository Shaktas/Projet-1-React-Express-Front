...
const Card = ({ name, url, username, password, pasteHandler }) => {
  ...
  return (
    <div className="min-w- rounded-3xl overflow-hidden text-blue-12 bg-blue-4 border border-blue-12 p-8 m-4">
      ...
      <p className="text-blue-9 text-base">
        <button role="button" onClick={pasteHandler}>
          <strong className="text-blue-12">Username :</strong> {username.length < 16 ? username : username.slice(0, 16) + '...'}
        </button>
      </p>
      <p className="text-blue-9 flex justify-between text-base">
        <div>
          <strong className="text-blue-12">Password :</strong>{' '}
          {password.length < 16 ? (!isEye ? password : '•'.repeat(6)) : !isEye ? password.slice(0, 16) + '...' : '•'.repeat(6)}
        </div>
        {isEye ? <EyeButton clickHandler={clickHandler} /> : <EyeCloseButton clickHandler={clickHandler} />}
      </p>
    </div>
  );
};
